...
void main()
{
	...
	while(1)
	{
		...
		{		
			...
			{	
				ch_buf[0] = ((uint)rece_buf[2] << 8) | rece_buf[1];//两个8位数据组合成16位数据
				ch_buf[1] = ((uint)rece_buf[4] << 8) | rece_buf[3];
				ch_buf[2] = ((uint)rece_buf[6] << 8) | rece_buf[5];
				ch_buf[3] = ((uint)rece_buf[8] << 8) | rece_buf[7];
				ch_buf[0] = ch_buf[0] >> 1;//16位数据中，低12位是ADC有效数据，右移一位，变为11位有效数据，用于sbus
				ch_buf[1] = ch_buf[1] >> 1;
				ch_buf[2] = ch_buf[2] >> 1;
				ch_buf[3] = ch_buf[3] >> 1;
				...
				send_buf[0] = 0x0F; //读数据要注意是高位在前还是低位在前
